    ...
    function(req, res) {
        ...
        if (req.user.data.AuthenticationResponseData.UserId == reqData.UserId) {
            ...
            try {
                ...
                //throw new Error('error');
                const connection = new db();
                var query = `SELECT * from users.fn_get_qpwise_assessor_certification_status_count_data(${reqData.SectorId},${reqData.UserId},${reqData.UserRoleId})`;
                connection.Query_Function(query, function(varlistData) {
                    response.QPwiseAssessorCertificationStatusCountData.StatusId = 1;
                    response.QPwiseAssessorCertificationStatusCountData.Message =
                        "Success";
    ...
    function(req, res) {
        ...
        if (req.user.data.AuthenticationResponseData.UserId == reqData.UserId) {
            ...
            try {
                ...
                var query;
                if (reqData.QualificationPackId)
                    query = `SELECT * from users.fn_get_assessor_certification_detailed_data(${reqData.SectorId},${reqData.QualificationPackId},${reqData.SearchType},${reqData.StateId},${reqData.UserId},${reqData.UserRoleId})`;
                else
                    query = `SELECT * from users.fn_get_assessor_certification_detailed_data(${reqData.SectorId},0,${reqData.SearchType},${reqData.StateId},${reqData.UserId},${reqData.UserRoleId})`;
                connection.Query_Function(query, function(varlistData) {
                    response.AssessorCertificationDetailedData.StatusId = 1;
                    ...
                    varlistData.forEach((element) => {
                        response.AssessorCertificationDetailedData.AssessorData.push({
                            ...
                            SscCertificateFileName: element["ssc_certification_file_name"] ?
                                element["ssc_certification_file_name"] : "",
                            ...
                            LanguagesKnown: element["language_known"] ?
                                element["language_known"] : "",
                            ...
                            EducationCertificateFileName: element[
                                    "education_certificate_file_name"
                                ] ?
                                element["education_certificate_file_name"] : "",
                            ExperienceCertificateFileName: element[
                                    "experience_certificate_file_name"
                                ] ?
                                element["experience_certificate_file_name"] : "",
                        });
                    });
                    log_info(
                        "Ended",
                        ...
                        if (index > 0) {
                            if (
                                ...
                                if (
                                    ...
                                    const index2 = response.PracticalAssessmentEvaluationData.Sections.findIndex(
                                        (datas) => datas.SectionId == element["section_id"]
                                    );
                ...
                connection.Query_Function(query, function(varlistData) {
                    ...
                    varlistData.forEach((element, index) => {
                        response.CandidateAssessmentEventData.push({
                            ...
                            SecondDifference: index == 0 ?
                                0 :
                                (new Date(element["event_date_time"]).getTime() -
                                    new Date(
                                        varlistData[index - 1]["event_date_time"]
                                    ).getTime()) /
                                1000,
                            FormattedSecondDifference: index == 0 ?
                                "0s" :
                                (new Date(element["event_date_time"]).getTime() -
                                    new Date(
                                        varlistData[index - 1]["event_date_time"]
...
router.post(
    "/GetCandidateAssessmentImageDataRequest",
    passport.authenticate("jwt", { session: false }),
    function(req, res) {
        var response = {
            StatusId: 0,
        ...
        if (req.user.data.AuthenticationResponseData.UserId == reqData.UserId) {
            ...
            try {
                ...
                const connection = new db();
                var query;
                query = `SELECT * from assessments.fn_get_candidate_assessment_image_data(${reqData.CandidateId},${reqData.RequestId},${reqData.ScheduleId},${reqData.AssessmentId},${reqData.ImageTypeId})`;
                connection.Query_Function(query, function(varlistData) {
                    response.StatusId = 1;
                    response.Message = "Success";
                    var location = "https://www.google.com/maps/search/?api=1&query=";
...
router.post(
    "/GetAssessorAssessmentDataRequest",
    passport.authenticate("jwt", { session: false }),
    function(req, res) {
        var response = {
        ...
        if (req.user.data.AuthenticationResponseData.UserId == reqData.UserId) {
            ...
            try {
                ...
                const connection = new db();
                var query;
                query = `SELECT * from assessments.fn_get_assessor_assessment_data(${reqData.UserId},${reqData.UserRoleId},${reqData.RequestType})`;
                connection.Query_Function(query, function(varlistData) {
                    response.AssessorAssessmentData.StatusId = 1;
